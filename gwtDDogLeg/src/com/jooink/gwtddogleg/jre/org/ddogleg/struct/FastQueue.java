...
public class FastQueue<T> {
	...
	protected void init(int initialMaxSize, Class<T> type, boolean declareInstances) {
		...
		if("org.ddogleg.nn.alg.KdTreeResult".equals(type.getName())) {
			data = (T[]) new KdTreeResult[initialMaxSize];
		} else {
			Window.alert("TypeName:"+type.getName());
			System.err.println("TypeName:"+type.getName());
		}
...
//		T []data = (T[])Array.newInstance(type, length);
		if("org.ddogleg.nn.alg.KdTreeResult".equals(type.getName())) {
			T[] data = (T[]) new KdTreeResult[length];
		} else {
			Window.alert("TypeName:"+type.getName());
			System.err.println("TypeName:"+type.getName());
		}
